...
export default function TicketDetail() {
  ...
  useEffect(() => {
    if (selectedTicketId && pb.authStore.isValid) {
      refreshData(true);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedTicketId]);

  // -- PocketBase Realtime (only when authenticated) --
  useEffect(() => {
    if (!selectedTicketId || !pb.authStore.isValid) return;
    ...
    const doSubscribe = async () => {
      try {
        await pb.collection('tickets').subscribe(selectedTicketId, () => {
          if (mounted) refreshData(true);
        });
      } catch { /* ignorar */ }

      try {
        await pb.collection('ticket_comentarios').subscribe('*', (e) => {
          if (mounted && e.record.ticket_id === selectedTicketId) {
            refreshData(true);
          }
        });
      } catch { /* ignorar */ }
    ...
    return () => {
      mounted = false;
      pb.collection('tickets').unsubscribe(selectedTicketId).catch(() => {});
