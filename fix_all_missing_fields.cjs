async function run() {
    try {
        ...
        const updateCollection = async (name, newFields) => {
            ...
            if (upRes.status !== 200) console.log(await upRes.json());
        };

        // Departamentos
        await updateCollection('departamentos', [
        ...
        await updateCollection('tickets', [
           ...
           { name: 'cliente_id', type: 'relation', collectionId: 'users', maxSelect: 1 },
           { name: 'agente_id', type: 'relation', collectionId: 'users', maxSelect: 1 },
           { name: 'departamento_id', type: 'relation', collectionId: 'departamentos', maxSelect: 1 },
           { name: 'fecha_resolucion', type: 'date' }
        ]);

        // Ticket Comentarios
        await updateCollection('ticket_comentarios', [
            { name: 'ticket_id', type: 'relation', collectionId: 'tickets', maxSelect: 1, required: true },
            { name: 'usuario_id', type: 'relation', collectionId: 'users', maxSelect: 1, required: true },
